...
dat_log_impute_merge <- merge(x = dat_log_impute_long,
                              y = na_pos_long[, c("SiteID", "imputed")],
                              by = "SiteID")
...
DSK_enr <- df_ratios %>%
  ...
  select(contains("log2_ratio_LH_DSK_")) %>%
  rownames_to_column(var ="SiteID") %>% 
  left_join(x = ., 
            y = dat_log_filter %>%
              as.data.frame() %>%
              select(starts_with(c("DSK_"))) %>%
  ...
  left_join(x = ., 
            y = dat_log %>%
              as.data.frame() %>%
              select(starts_with(c("NA_count_DSK_L"))) %>%
              rownames_to_column(var = "SiteID"),
            by = "SiteID") %>% 
  left_join(x = ., 
            y = psites %>%
              select(c("Site", "Leading.proteins","Gene.names", "Amino.acid", 
...
DS_enr <- df_ratios %>%
  ...
  select(contains("log2_ratio_LH_DS_")) %>%
  rownames_to_column(var ="SiteID") %>% 
  left_join(x = ., 
            y = dat_log_filter %>%
              as.data.frame() %>%
              select(starts_with(c("DS_"))) %>%
              rename_with(.cols = everything(), 
                          function(x){paste0("log2_intensity_", x)}) %>%
              rownames_to_column(var = "SiteID"),
            by = "SiteID") %>% 
  left_join(x = ., 
            y = dat_log %>%
              as.data.frame() %>%
              select(starts_with(c("NA_count_DS_L"))) %>%
              rownames_to_column(var = "SiteID"),
            by = "SiteID") %>% 
  left_join(x = ., 
            y = psites %>%
...
SK_enr <- df_ratios %>%
  ...
  select(contains("log2_ratio_LH_SK_")) %>%
  rownames_to_column(var ="SiteID") %>% 
  left_join(x = ., 
            y = dat_log_filter %>%
              as.data.frame() %>%
              select(starts_with(c("SK_"))) %>%
              rename_with(.cols = everything(), 
                          function(x){paste0("log2_intensity_", x)}) %>%
              rownames_to_column(var = "SiteID"),
            by = "SiteID") %>% 
  left_join(x = ., 
            y = dat_log %>%
              as.data.frame() %>%
              select(starts_with(c("NA_count_SK_L"))) %>%
              rownames_to_column(var = "SiteID"),
            by = "SiteID")  %>% 
  left_join(x = ., 
            y = psites %>%
...
K_enr <- df_ratios %>%
  ...
  select(contains("log2_ratio_LH_K_")) %>%
  rownames_to_column(var ="SiteID") %>% 
  left_join(x = ., 
            y = dat_log_filter %>%
              as.data.frame() %>%
  ...
  left_join(x = ., 
            y = dat_log %>%
              as.data.frame() %>%
              select(starts_with(c("NA_count_K_L"))) %>%
              rownames_to_column(var = "SiteID"),
            by = "SiteID") %>% 
  left_join(x = ., 
            y = psites %>%
...
#Add the original L/H-ratios used to determine biotin-enriched peptides
psite_enr_table <-
  left_join(x = ., 
            y = df_ratios %>%
              as.data.frame() %>%
...
dat_norm_impute_merge <- merge(x = dat_norm_impute_long,
                              y = na_pos_long_2[, c("SiteID", "imputed")],
                              by = "SiteID")
...
## Combined DE-analysis output workbook####
# Write xlsx workbook which combines prot_tables and the candidate tables
# of differential expressed (DE) proteins
wbook <- createWorkbook("P-sites_Results")
addWorksheet(wbook, "Identified_P-sites")
addWorksheet(wbook, "Quantified_P-sites")
addWorksheet(wbook, "Enriched_P-sites")
...
writeData(wbook, sheet = 6, SK_enr)
writeData(wbook, sheet = 7, K_enr)
writeData(wbook, sheet = 8, candidates_DSK_DS)
writeData(wbook, sheet = 9, candidates_DSK_SK)
writeData(wbook, sheet = 10, candidates_DSK_K)
writeData(wbook, sheet = 11, candidates_DS_SK)
writeData(wbook, sheet = 12, candidates_DS_K)
writeData(wbook, sheet = 13, candidates_SK_K)
saveWorkbook(wbook,
             paste0(DE_path, "Enriched_P-sites_and_DE_results_",
                    projectname,"_",format(Sys.time(), '%Y%m%d_%H%M%S'),
                    ".xlsx"),
...
Dclk1_sites_ma <- Dclk1_sites_ma %>% 
  select(contains(c("norm_intensity","Site2")))
...
## Set relative output path####
DE_annot_path <- "DE_analysis/Annotation_enrichment/"
