...
# Imputation-------------------------------------------------------------------
# We impute missing values of light inetsities to get more robust statistics
...
candidates_DSK_DS <- topTable(fit3, coef = "DSK-DS", number = Inf) %>%
  ...
  left_join(x = ., 
            y = protein %>%
              select(c("Majority.protein.IDs", "Gene.names", "Fasta.headers")))
...
## Abundance table####
#Generate protein table with abundance values from normalized matrix and 
#  norm+imputed matrix
...
writeData(wbook, sheet = 1, prot_table)
...
saveWorkbook(wbook,
             paste0(DE_path, "Enriched_ProteinGroups_and_DE_results_",
...
## GSEA analysis#### DOES NOT WORK YET!!!!
### Generate geneList 
