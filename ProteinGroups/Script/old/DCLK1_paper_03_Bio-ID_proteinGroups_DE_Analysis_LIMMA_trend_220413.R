...
#------------------------------------------------------------------------------
#We impute missing values again to get more robust statistics
...
heatanno$Construct <- factor(heatanno$Construct, levels = c("DS", "DSK", "K", "SK"))
...
saveWorkbook(wbook,
             paste0("Enriched_ProteinGroups_and_DE_results_",
                    projectname,"_",format(Sys.time(), '%Y%m%d_%H%M%S'),
                    ".xlsx"),
             overwrite = TRUE)

#_______________________________________________________________
#Perform GO-enrichment and GSEA analyses with the clusterProfiler package
...
#Perform GO over-representation analyses
  ego_up_BP <- enrichGO(
    gene          = up_ids,
  ...
  #Simplify GO-Terms by removing redundant terms (semantic similarity)
  
  ego_up_BP <- try(pairwise_termsim(ego_up_BP))
  if(isTRUE(class(ego_up_BP) != "try-error")) 
  {
  ...
  ego_up_CC <- try(pairwise_termsim(ego_up_CC))
  ego_up_CC_simple <- try(clusterProfiler::simplify(ego_up_CC, 
                                                cutoff=0.7, 
                                                by="p.adjust", 
                                                select_fun=min))
  
  ego_up_MF <- try(pairwise_termsim(ego_up_MF))
  ego_up_MF_simple <- try(clusterProfiler::simplify(ego_up_MF, 
                                                cutoff=0.7, 
                                                by="p.adjust", 
                                                select_fun=min))
  
  ego_down_BP <- try(pairwise_termsim(ego_down_BP))
  ego_down_BP_simple <- try(clusterProfiler::simplify(x = ego_down_BP, 
                                                  cutoff=0.7, 
                                                  by="p.adjust", 
                                                  select_fun=min))
  
  ego_down_CC <- try(pairwise_termsim(ego_down_CC))
  ego_down_CC_simple <- try(clusterProfiler::simplify(x = ego_down_CC, 
                                                  cutoff=0.7, 
                                                  by="p.adjust", 
                                                  select_fun=min))
  
  ego_down_MF <- try(pairwise_termsim(ego_down_MF))
  ego_down_MF_simple <- try(clusterProfiler::simplify(x = ego_down_MF, 
                                                  cutoff=0.7, 
                                                  by="p.adjust", 
                                                  select_fun=min))
  
  #Write xlsx workbook which combines the different GO tables both complete
  #and simplyfied
  ...
  saveWorkbook(wbook,
               paste0("GO_overrepresentation_ProteinGroups_", i, "_", 
                      projectname,"_",format(Sys.time(), '%Y%m%d_%H%M%S'),
                      ".xlsx"),
               overwrite = TRUE)
}
