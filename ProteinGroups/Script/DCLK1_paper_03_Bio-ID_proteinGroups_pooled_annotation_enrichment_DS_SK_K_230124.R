...
# DS vs SK
DS_SK_up <- filter(candidates_DS_SK,
                    logFC > 0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Gene.names"))
DS_SK_up <- str_split(DS_SK_up$Gene.names, ";", simplify = TRUE) %>%
  as.data.frame() %>% 
  dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
DS_SK_up <- DS_SK_up$V1
DS_SK_up <- na.omit(DS_SK_up)

DS_SK_down <- filter(candidates_DS_SK,
                      logFC < -0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Gene.names"))
DS_SK_down <- str_split(DS_SK_down$Gene.names, ";", simplify = TRUE) %>%
  as.data.frame() %>% 
  dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
DS_SK_down <- DS_SK_down$V1
DS_SK_down <- na.omit(DS_SK_down)

#DS vs K
DS_K_up <- filter(candidates_DS_K,
                   logFC > 0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Gene.names"))
DS_K_up <- str_split(DS_K_up$Gene.names, ";", simplify = TRUE) %>%
  as.data.frame() %>% 
  dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
DS_K_up <- DS_K_up$V1
DS_K_up <- na.omit(DS_K_up)

DS_K_down <- filter(candidates_DS_K,
                     logFC < -0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Gene.names"))
DS_K_down <- str_split(DS_K_down$Gene.names, ";", simplify = TRUE) %>%
  as.data.frame() %>% 
  dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
DS_K_down <- DS_K_down$V1
DS_K_down <- na.omit(DS_K_down)

#### Combine DS_SK & DS_K ####
DS_SK_K_up <- unique(c(DS_SK_up,DS_K_up))
DS_SK_K_down <- unique(c(DS_SK_down,DS_K_down))
...
#### Perform over-representation analyses####
  ...
  saveWorkbook(wbook,
               paste0(annot_path,"GO_overrep_", "DS_SK_K", "_"
                      ,"pVal<0.05_qVal<0.2_Top30_", 
...
#### Generate DE-protein lists####
# DS vs SK
DS_SK_up_uniprot_ids <- filter(candidates_DS_SK,
                    logFC > 0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Majority.protein.IDs"))
DS_SK_up_uniprot_ids <- str_split(DS_SK_up_uniprot_ids$Majority.protein.IDs, ";", simplify = TRUE) %>%
  as.data.frame() %>% 
  dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
DS_SK_up_uniprot_ids <- DS_SK_up_uniprot_ids$V1
DS_SK_up_uniprot_ids <- na.omit(DS_SK_up_uniprot_ids)

DS_SK_down_uniprot_ids <- filter(candidates_DS_SK,
                      logFC < -0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Majority.protein.IDs"))
DS_SK_down_uniprot_ids <- str_split(DS_SK_down_uniprot_ids$Majority.protein.IDs, ";", simplify = TRUE) %>%
  as.data.frame() %>% 
  dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
DS_SK_down_uniprot_ids <- DS_SK_down_uniprot_ids$V1
DS_SK_down_uniprot_ids <- na.omit(DS_SK_down_uniprot_ids)

#DS vs K
DS_K_up_uniprot_ids <- filter(candidates_DS_K,
                   logFC > 0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Majority.protein.IDs"))
DS_K_up_uniprot_ids <- str_split(DS_K_up_uniprot_ids$Majority.protein.IDs, ";", simplify = TRUE) %>%
  as.data.frame() %>% 
  dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
DS_K_up_uniprot_ids <- DS_K_up_uniprot_ids$V1
DS_K_up_uniprot_ids <- na.omit(DS_K_up_uniprot_ids)

DS_K_down_uniprot_ids <- filter(candidates_DS_K,
                     logFC < -0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Majority.protein.IDs"))
DS_K_down_uniprot_ids <- str_split(DS_K_down_uniprot_ids$Majority.protein.IDs, ";", simplify = TRUE) %>%
  as.data.frame() %>% 
  dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
DS_K_down_uniprot_ids <- DS_K_down_uniprot_ids$V1
DS_K_down_uniprot_ids <- na.omit(DS_K_down_uniprot_ids)

#### Combine DS_SK & DS_K ####
DS_SK_K_up_uniprot_ids <- unique(c(DS_SK_up_uniprot_ids,DS_K_up_uniprot_ids))
DS_SK_K_down_uniprot_ids <- unique(c(DS_SK_down_uniprot_ids,DS_K_down_uniprot_ids))


  #### Perform over-representation analyses####
  
  ego_down_KEGG <- enrichKEGG(
    gene          = DS_SK_K_down_uniprot_ids,
  ...
  saveWorkbook(wbook,
               paste0(annot_path,"KEGG_overrep_", "DS_SK_K", "_"
                      ,"pVal<0.05_qVal<0.2_Top30_", 
                      projectname,"_",format(Sys.time(), '%Y%m%d_%H%M%S'),
                      ".xlsx"),
               overwrite = TRUE)
