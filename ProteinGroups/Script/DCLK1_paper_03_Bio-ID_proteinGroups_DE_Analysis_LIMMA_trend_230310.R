...
rownames(protein) <- protein$Majority.protein.IDs
...
count_tbl <- data.frame(type = "identified",
                        count = nrow(protein),
                        details = "Number of identified proteinGroups in original MQ results txt table")
...
count_tbl[nrow(count_tbl)+1,] <- c("identified_wo_rev_cont_site",
                                   nrow(p_filter),
                                   "Number of identified sites after removal of reverse, contaminants and identified by site")
...
count_tbl[nrow(count_tbl)+1,] <- c("2_valid_L_val_anygroup",
                                   nrow(dat_log_filter),
                                   "ProteinGroups quantified in light channel for at least 2 times in any sample group")
...
dat_log_impute_merge <- merge(x = dat_log_impute_long,
                              y = na_pos_long[, c("sampleID", "ProtID", "imputed")],
                              by = c("sampleID", "ProtID"))
...
ggsave(
  paste0(QC_path,
    "Density_log2_intensities+impute_L+H_400x200_",
    projectname,
    "_",
    format(Sys.time(), '%Y%m%d_%H%M%S'),
    ".png"
...
DSK_enr <- df_ratios %>%
  ...
  left_join(x = ., 
            y = protein %>%
              dplyr::select(c("Majority.protein.IDs", "Protein.names", 
                              "Gene.names", "Fasta.headers")) %>%
              dplyr::rename(ProtID = "Majority.protein.IDs"),
            by = "ProtID")
...
count_tbl[nrow(count_tbl)+1,] <- c("enriched",
                                   ...
                                   "ProteinGroups having at least 2 log2 L/H ratios >1 in any sample group")
...
## Abundance table####
### Identified proteins####
# Generate protein table with abundance values from log2 abundance matrix
# Add log2 abundances
prot_id_table <-
  merge(
    x = protein %>%
      dplyr::select(c("Protein.IDs",
    ...
    y = dat_log %>%
      ...
      rename_with(.cols = everything(), 
                  function(x){paste0("log2_intensity_", x)}) %>%
      rownames_to_column(var = "Majority.protein.IDs"),
    by = "Majority.protein.IDs"
  )


### Quantified proteins####
# Generate proteinGroup table with abundance values from log2_filter abundance matrix
# Add log2_filter abundances. These are proteinGroups quantified with 2 valid values 
...
### Enriched proteins ####
# Generate proteinGroups table with the L/H-ratios and the abundance values from the 
# log2_enriched abundance matrix 
prot_enr_table <-
  merge(
    x = protein %>%
      dplyr::select(c("Protein.IDs",
...
# Write xlsx workbook which combines prot_tables and the candidate tables
# of biotin-enriched proteins
wbook <- createWorkbook("ProteinGroups_Biotin-Enriched_Results")
addWorksheet(wbook, "Identified_ProteinGroups")
addWorksheet(wbook, "Quantified_ProteinGroups")
addWorksheet(wbook, "Enriched_ProteinGroups")
addWorksheet(wbook, "enr_DSK")
addWorksheet(wbook, "enr_DS")
addWorksheet(wbook, "enr_SK")
addWorksheet(wbook, "enr_K")
writeData(wbook, sheet = 1, prot_id_table)
writeData(wbook, sheet = 2, prot_quant_table)
writeData(wbook, sheet = 3, prot_enr_table)
...
# Set seed to always get the same imputations when running the script 
set.seed(1234)
...
dat_norm_impute_merge <- merge(x = dat_norm_impute_long,
                              y = na_pos_long_2[, c("sampleID","id", "imputed")],
                              by = c("sampleID", "id"))
...
ggsave(
  paste0(QC_path,
    "Boxplot_enriched_norm_log2_intensities+impute_L_400x150_",
    projectname,
    "_",
    format(Sys.time(), '%Y%m%d_%H%M%S'),
    ".png"
...
ggsave(
  paste0(QC_path,
    "Density_enriched_norm_log2_intensities+impute_L_200x200_",
    projectname,
    "_",
    format(Sys.time(), '%Y%m%d_%H%M%S'),
    ".png"
...
for (i in x) {
  ...
  limma_result$GeneID <- protein[rownames(limma_result), "Gene.names"]
...
candidates_DSK_DS <- all_DSK_DS %>%
  ...
  left_join(x = ., 
            y = protein %>%
              dplyr::select(c("Majority.protein.IDs", "Gene.names", "Fasta.headers")))
...
prot_norm_table <-
  merge(
    ...
    y = all_DSK_DS %>%
      ...
      rownames_to_column(var = "Majority.protein.IDs") %>%
      dplyr::mutate(significant_DSK_DS = Majority.protein.IDs %in% 
                      candidates_DSK_DS$Majority.protein.IDs),
    by = "Majority.protein.IDs") %>%
    left_join(x = .,
            y = all_DSK_SK %>%
              as.data.frame() %>%
              ...
              rownames_to_column(var = "Majority.protein.IDs") %>%
              dplyr::mutate(significant_DSK_SK = Majority.protein.IDs %in% 
                              candidates_DSK_SK$Majority.protein.IDs),
            by = "Majority.protein.IDs") %>%
  left_join(x = .,
            y = all_DSK_K %>%
              as.data.frame() %>%
              ...
              rownames_to_column(var = "Majority.protein.IDs") %>%
              dplyr::mutate(significant_DSK_K = Majority.protein.IDs %in% 
                              candidates_DSK_K$Majority.protein.IDs),
            by = "Majority.protein.IDs") %>%
  left_join(x = .,
            y = all_DS_SK %>%
              as.data.frame() %>%
              ...
              rownames_to_column(var = "Majority.protein.IDs") %>%
              dplyr::mutate(significant_DS_SK = Majority.protein.IDs %in% 
                              candidates_DS_SK$Majority.protein.IDs),
            by = "Majority.protein.IDs") %>%
  left_join(x = .,
            y = all_DS_K %>%
              as.data.frame() %>%
              ...
              rownames_to_column(var = "Majority.protein.IDs") %>%
              dplyr::mutate(significant_DS_K = Majority.protein.IDs %in% 
                              candidates_DS_K$Majority.protein.IDs),
            by = "Majority.protein.IDs") %>%
  left_join(x = .,
            y = all_SK_K %>%
              as.data.frame() %>%
              ...
              rownames_to_column(var = "Majority.protein.IDs") %>%
              dplyr::mutate(significant_SK_K = Majority.protein.IDs %in% 
                              candidates_SK_K$Majority.protein.IDs),
            by = "Majority.protein.IDs")
...
wbook <- createWorkbook("ProteinGroup_Results")
addWorksheet(wbook, "Enriched_ProteinGroups_norm")
...
writeData(wbook, sheet = 1, prot_norm_table)
...
# Regulated proteinGroups heatmaps####----------------------------------------
## Filter prot_table for all regulated proteins ####
prot_regulated <- prot_norm_table %>% 
  filter(Majority.protein.IDs %in% 
           c(candidates_DSK_DS$Majority.protein.IDs,
             candidates_DSK_K$Majority.protein.IDs,
             candidates_DSK_SK$Majority.protein.IDs,
             candidates_DS_K$Majority.protein.IDs,
             candidates_DS_SK$Majority.protein.IDs,
...
count_tbl[nrow(count_tbl)+1,] <- c("regulated",
                                   nrow(prot_regulated),
                                   "ProteinGroups showing significant regulation in any pairwise group comparison")

### Extract only intensities ####
prot_reg_matrix <-prot_regulated %>%
  dplyr::select(contains("norm_imputed_intensity")) 

### Set rownames ####
rownames(prot_reg_matrix) <- prot_regulated$Majority.protein.IDs

#### Set colnames needed for pheatmap col_labels ####
colnames(prot_reg_matrix) <- c("DS_L_01",
                               "DS_L_02",
                               "DS_L_03",
                               "DSK_L_01",
                               "DSK_L_02",
...
### Generate Heatmap####
#Generate annotation table
heatanno <- data.frame(sample = colnames(prot_reg_matrix))
heatanno$Construct <- rep(c("DS", "DSK", "K", "SK"),
                          each = 3)
rownames(heatanno) <- colnames(prot_reg_matrix)
heatanno <- subset (heatanno, select = -sample)
heatanno$Construct <- factor(heatanno$Construct, levels = c("DSK", 
                                                            "DS", 
                                                            "SK", 
                                                            "K"))
...
wbook <- createWorkbook("ProteinGroups_ID-Count_table")
...
################################################################################
# Annotation-enrichment analysis of DE-proteins#################################
################################################################################
# Perform GO and KEGG--overrepresentation  with the clusterProfiler 
# package for the proteins up- and downregulated in each pairwise comparison
...
prot_norm_table <- openxlsx::read.xlsx(xlsxFile = wbook, sheet = 1)
...
background_prot_ids <- prot_norm_table$Gene.names %>%
...
for (i in x) {
  ...
  up_ids <- filter(limma_result,
               logFC > 0.58 & adj.P.Val < 0.05 ) %>%
    dplyr::select(c("GeneID"))
  up_ids <- str_split(up_ids$GeneID, ";", simplify = TRUE) %>%
    as.data.frame() %>% 
    dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
  up_ids <- up_ids$V1
  up_ids <- na.omit(up_ids)
  
  down_ids <- filter(limma_result,
               logFC < -0.58 & adj.P.Val < 0.05 ) %>%
    dplyr::select(c("GeneID"))
  down_ids <- str_split(down_ids$GeneID, ";", simplify = TRUE) %>%
    as.data.frame() %>% 
    dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
...
### Over-representation analysis ####
#### Generate background list####
background_uniprot_ids <- prot_norm_table$Majority.protein.IDs %>%
  str_split(";", simplify = TRUE) %>%
  as.data.frame()  %>% 
  dplyr::mutate(across(everything(), ~ifelse(.=="", NA, as.character(.))))
...
# Venn of DE-proteins ##########################################################
################################################################################
# We make two comparisons a) proteins up in microtubule-located constructs
# (DSK-SK up, DSK-K up, DS-SK up, DS-K up) and b) proteins up in nuclear 
# constructs (DSK-SK down, DSK-K down, DS-SK down, DS-K down)

## Extract gene names for each regulation ####
### Microtubule up ####
DSK_SK_up_ids <- candidates_DSK_SK %>%
  filter(logFC > 0.58 & adj.P.Val < 0.05 ) %>%
...
### Nuclear up ####
DSK_SK_down_ids <- candidates_DSK_SK %>%
  filter(logFC < -0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Gene.names")) %>% 
  separate_wider_delim(cols = everything(), 
                       delim = ";",
...
DSK_K_down_ids <- candidates_DSK_K %>%
  filter(logFC < -0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Gene.names")) %>% 
  separate_wider_delim(cols = everything(), 
                       delim = ";",
...
DS_SK_down_ids <- candidates_DS_SK %>%
  filter(logFC < -0.58 & adj.P.Val < 0.05 ) %>%
  dplyr::select(c("Gene.names")) %>% 
  separate_wider_delim(cols = everything(), 
                       delim = ";",
...
venn_mt <- list( DSK_SK = DSK_SK_up_ids$Gene.names,
                       DSK_K = DSK_K_up_ids$Gene.names,
                       DS_SK = DS_SK_up_ids$Gene.names,
                       DS_K = DS_K_up_ids$Gene.names)

## Create lists for each Venn to be ploted ####
venn_nu <- list( SK_DSK = DSK_SK_down_ids$Gene.names,
                 K_DSK = DSK_K_down_ids$Gene.names,
                 SK_DS = DS_SK_down_ids$Gene.names,
                 K_DS = DS_K_down_ids$Gene.names)
...
venn.diagram(venn_mt,
             filename = paste0("Venn_all-up_MT_",projectname,
                               "_",format(Sys.time(), '%Y%m%d_%H%M%S'),".png"),
             main = "Venn all up - microtubules",
             category.names = c(DSK_SK = "DSK vs. SK",
                                DSK_K = "DSK vs. K",
                                DS_SK = "DS vs. SK",
                                DS_K = "DS vs. K"),
             print.mode = c("raw", "percent"),
             sigdigs = 1,
             fill = c("#D43F3AFF","#EFC000FF", "#5CB85CFF", "#9632B8FF"),
             fontfamily = "sans",
             ...
             cat.fontfamily = "sans",
             cat.fontface = "bold",
             cat.col = c("#D43F3AFF","#EFC000FF", "#5CB85CFF", "#9632B8FF"),
             imagetype = "png")

venn.diagram(venn_nu,
             filename = paste0("Venn_all-up_NUC_",projectname,
                               "_",format(Sys.time(), '%Y%m%d_%H%M%S'),".png"),
             main = "Venn all up - nucleus",
             category.names = c(SK_DSK = "SK vs. DSK",
                                K_DSK = "K vs. DSK",
                                SK_DS = "SK vs. DS",
                                K_DS = "K vs. DS"),
             print.mode = c("raw", "percent"),
